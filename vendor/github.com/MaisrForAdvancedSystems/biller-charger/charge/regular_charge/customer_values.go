		...
		if services != nil && len(services) > 0 {
			...
			for _, srv := range services {
				...
				if srv.Connection != nil {
					...
					mainMappedData.cType = conn.CType.CType
					mainMappedData.cTypeGroup = conn.CType.CTypeGroupid
					...
					if typ == ENTITY_TYPE_CTYPE {
						...
						if srv.Connection.SubConnections != nil && len(srv.Connection.SubConnections) > 0 {
							for idx := range srv.Connection.SubConnections {
								...
								values.append(srv.Connection.SubConnections[idx].CType.CType, subMappedType)
							}
						} else {
							values.append(srv.Connection.CType.CType, mainMappedData)
						}
					}
					if typ == ENTITY_TYPE_CTYPE_GROUP {
						if srv.Connection.SubConnections != nil && len(srv.Connection.SubConnections) > 0 {
							for idx := range srv.Connection.SubConnections {
								...
								values.append(srv.Connection.SubConnections[idx].CType.CTypeGroupid, subMappedType)
								log.Println("CTYPE_GROUPS ", srv.ServiceType, srv.Connection.SubConnections[idx].CType.CTypeGroupid)
							}
						} else {
							values.append(srv.Connection.CType.CTypeGroupid, mainMappedData)
							if srv.Connection.CType.CTypeGroupid != nil {
								log.Println("CTYPE_GROUP ", srv.ServiceType, *srv.Connection.CType.CTypeGroupid)
							}
						}
					}
